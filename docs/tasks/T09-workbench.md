...
P03 在独立8768页面增加出行条件、地点确认、有限主动地图查询及时间缺口，未配置时仍可保存/采用/恢复。不是内嵌地图SDK或完整工作台。见 [P03设计](../architecture/p03-amap-route-check.md)。

P01 仅交付 Vue 同源本机缓存页面、条件选择与改选闭环，保持来源/缺口可见；地图、预算、RAG、SSE 和发行工作台未完成。见 [P01 范围](../architecture/p01-cached-preview.md)。
