...
P03 增加当前对象的出行输入、明确假设下的部分时间检查、修改/取消/采用和恢复；未完成费用、完整行程求解或可执行性验证。见 [P03设计](../architecture/p03-amap-route-check.md)。

P01 仅实现兴趣选择的预览、确认、取消和 revision 持久恢复；不做时间／费用求解、锁定供应商或路线可行性验证，原任务未整体完成。见 [P01 范围](../architecture/p01-cached-preview.md)。
