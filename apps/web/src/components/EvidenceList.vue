...
<template>
  <article v-for="row in rows" :key="row.claim_id" class="evidence-item">
    ...
    <details class="lineage"><summary>查看引用定位与审核记录</summary>
      <p>审核：{{ row.review_status === 'LOCAL_REVALIDATION' ? '已存模型提议的本地重校验；模型未重新作答，当前事实未核实' : row.review_status === 'MODEL_CONTEXT_REVIEWED' ? '模型上下文审核 + 程序引用复核；未核实当前事实和可行性' : '历史开发 Work 辅助审核，非自动语义审核' }}</p>
