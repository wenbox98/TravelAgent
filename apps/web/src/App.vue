...
async function load() {
  await run(async ticket => {
    ...
    if (ticket !== generation) return
    researches.value = result.researches; mode.value = result.mode
...
<template>
  <main>
    ...
    <p class="notice">缓存驱动开发预览；尚未核实当前行程可行性。本轮仅使用已审核的本地资料，不启动小红书、不调用模型。</p>
    ...
    <div v-if="view" class="workspace">
      ...
      <aside aria-label="选择与条件">
        ...
        <section class="card gaps"><p class="eyebrow">下一步要核实什么</p><h2>保留这些缺口</h2><ul><li v-for="gap in view.gaps" :key="gap">{{ gap }}</li></ul><p class="muted">本轮仅列待办，不派发查询。不会把来源七日草案删两天后当作五天可行行程。</p></section>
      </aside>
    </div>
    <section v-else-if="!busy && mode" class="card empty"><h2>先选一份已有研究</h2><p>页面将从数据库中的已审核证据生成草案。未给预算或人数，也能先看。</p></section>
    <footer>G0 仅保留历史验收；G1 仍 NOT PASS。此页面不是完整旅行产品或发布验收。没有探测当前小红书、模型、地图或报价服务。</footer>
  </main>
</template>
